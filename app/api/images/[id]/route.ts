...
import { withRouteHandler } from "@/lib/routeHandler";

// --- PUT Handler ---
const putHandler = async (req: NextRequest, params: Record<string, string>) => {
...
// --- POST Handler ---
const postHandler = async (req: NextRequest, params: Record<string, string>) => {
  const token = await getToken({ req, secret: process.env.NEXTAUTH_SECRET });
  if (!token?.sub) return new NextResponse("Unauthorized", { status: 401 });
