...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        const { metadata, link } = await req.json(); // <-- THE FIX: We now expect metadata and a link

        await dbConnect();
        
        const updatedImage = await Image.findOneAndUpdate(
            { _id: imageId, userId: token.sub },
            { $set: { metadata: metadata, link: link } }, // <-- THE FIX: Save both fields
            { new: true }
        );
...
// --- Function to REGENERATE an existing image with AI (already correct) ---
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        if (!aiFetchResponse.ok) { const err = await aiFetchResponse.text(); throw new Error(`API error: ${err}`); }
